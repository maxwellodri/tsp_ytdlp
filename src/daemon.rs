...
pub async fn run_daemon(config: Config) -> Result<()> {
    ...
    // Process queued tasks from offline queueing
    {
            ...
            && !queued.urls.is_empty()
        {
            ...
            for (url, media_type, download_dir) in queued.urls {
                // Check for duplicates
                if mgr.check_duplicate_url(&url).is_none() {
                    match mgr.add_task(url.clone(), media_type, download_dir) {
                        Ok(task_id) => {
...
async fn process_request(
    ...
    match request {
        ClientRequest::Add {
            ...
            match mgr.find_task_by_url(&url) {
                crate::TaskByUrl::Exists {
                    ...
                    let mut mgr = manager.lock().await;
                    match mgr.add_task(url_clone.clone(), media_type, download_dir_clone) {
                        Ok(task_id) => {
                            // Save tasks
                            if let Err(e) = mgr.save_tasks() {
                ...
                crate::TaskByUrl::DoesntExist => {
                    ...
                    match mgr.add_task(url.clone(), media_type, download_dir.clone()) {
                        Ok(task_id) => {
                            ...
                            crate::common::send_notification(
                                ...
                                &format!(
                                    "Task {} added{} for URL: {}",
                                    task_id, paused_suffix, url_clone
                                ),
                                Some(3000),
                                &config_clone,
                            )
        ...
        ClientRequest::Info { id } => {
            ...
            if let Some(task) = mgr.tasks.get_task(id) {
            ...
            match ids {
                ...
                None => {
                    ...
                    let paused_ids: Vec<u64> = mgr
                        .tasks
                        .iter()
                        .filter_map(|(id, task)| if task.is_paused() { Some(*id) } else { None })
                        .collect();
...
async fn task_processor_loop(manager: Arc<Mutex<TaskManager>>, config: Config) {
    ...
    loop {
        // PHASE 1: Try to spawn a task if we have capacity
            ...
            if let Some((task_id, task)) = next_task {
                ...
                match task {
                    crate::task::Task::Queued {
                        ...
                        if let Some(t) = mgr.tasks.get_task_mut(task_id) {
                            *t = crate::task::Task::GetName {
                                url: url.clone(),
                                media_type,
                                metadata: Some(crate::task::GetNameMetadata {
                                    title: None,
                                    expected_size_bytes: None,
                                    directory: String::new(),
                                }),
                                download_dir: download_dir_clone.clone(),
                            };
                            mgr.tasks
                    ...
                    crate::task::Task::GetName {
                        ...
                        let handle = tokio::spawn(async move {
                            let data_dir = crate::get_data_dir();
                            let log_file_path = data_dir.join(format!("task_{}.log", task_id));
                    ...
                    crate::task::Task::DownloadVideo {
                        ...
                        let title_display =
                            download_metadata.title.as_deref().unwrap_or("download");
                        ...
                        crate::common::send_notification(
                            ...
                            &format!(
                                "Resuming download: {}{} 🔄 {}",
                                title_display, dir_suffix, media_emoji
                            ),
                            Some(3000),
                            &config_clone,
                        )
        ...
        // PHASE 2: If at capacity or nothing to spawn, wait for a task to complete
                ...
                match task_result {
                    ...
                    Ok(Err(e)) => {
                        ...
                        crate::common::send_critical_notification(
                            &url,
                            &format!("❌ Download failed:{} {}", dir_suffix, e),
                            &config,
                        )
                        .await;
        ...
        // PHASE 3: Nothing to spawn and nothing active - brief sleep
        drop(mgr);
